...
#
# What is deployed is the Go service (#287, #304): one image, two roles per
# environment. Migrations are NOT rolled back. Keep them additive -- never drop
# or rename a column in the same release that ships code depending on it -- or
# an image rollback will meet a schema it cannot read.
...
# ownership before anything mounts it, and refuse to deploy if it is there but
# wrong.
...
do_deploy() {
  # First, before any work -- checkout_test.go asserts that. The branch comes
  ...
  info "deploying ${GO_IMAGE}:${sha} to ${env_name}"
  docker pull "${GO_IMAGE}:${sha}" >/dev/null \
  ...
  local resolved
  resolved=$(docker image inspect "${GO_IMAGE}:${sha}" \
  ...
  if [ "$resolved" != "$sha" ]; then
    ...
    docker tag "${GO_IMAGE}:${sha}" "${GO_IMAGE}:${resolved}"
    sha=$resolved
  fi

  env_set "$tag_key" "$sha"

  info "migrating ${env_name}'s PostgreSQL"
...
do_rollback() {
  ...
  local web fw web_port fw_port tag_key prev_file
  read -r web fw web_port fw_port tag_key prev_file _ <<<"$(target_for "$env_name")"
  local prev_path="${STATE_DIR}/${prev_file}"
